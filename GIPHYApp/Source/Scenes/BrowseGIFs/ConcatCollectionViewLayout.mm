...
#import "ConcatCollectionViewLayout.h"
//#import "ConcatCollectionViewLayoutInternal.h"
...
@interface ConcatCollectionViewLayoutInvalidationContext : UICollectionViewLayoutInvalidationContext
...
@implementation ConcatCollectionViewLayoutInvalidationContext
@end

@interface ConcatCollectionViewLayout ()
...
@implementation ConcatCollectionViewLayout {
...
- (CGSize)collectionViewContentSize {
    ...
    const CGFloat height = CGRectGetMaxY(section.bounds) + section.edgeInsets.bottom;
...
- (void)invalidateLayoutWithContext:(ConcatCollectionViewLayoutInvalidationContext *)context {
...
+ (Class)invalidationContextClass {
    return [ConcatCollectionViewLayoutInvalidationContext class];
}

- (UICollectionViewLayoutInvalidationContext *)invalidationContextForBoundsChange:(CGRect)newBounds {
    const CGRect oldBounds = self.collectionView.bounds;

    ConcatCollectionViewLayoutInvalidationContext *context =
    (ConcatCollectionViewLayoutInvalidationContext *)[super invalidationContextForBoundsChange:newBounds];
...
- (void)setStickyHeaderOriginYAdjustment:(CGFloat)stickyHeaderOriginYAdjustment {
    ...
    if (_stickyHeaderOriginYAdjustment != stickyHeaderOriginYAdjustment) {
        ...
        ConcatCollectionViewLayoutInvalidationContext *invalidationContext = [[ConcatCollectionViewLayoutInvalidationContext alloc] init];
...
- (void)cacheLayout {
    ...
    id<ConcatCollectionViewLayoutDelegate> myDelegate = self.delegate;
    bool myDelegateOverridesEdgeInset = [myDelegate respondsToSelector:@selector(collectionView:layout:edgeInsetForSectionAtIndex:)];
    ...
    for (NSInteger section = 0; section < sectionCount; section++) {
        ...
        const UIEdgeInsets edgeInsets = myDelegateOverridesEdgeInset
            ? [myDelegate collectionView:collectionView layout:self edgeInsetForSectionAtIndex:section]
            : insets;

        const CGFloat paddedWidth = width - edgeInsets.left - edgeInsets.right;
        ...
        itemX += (section > 0 ? insets.left : edgeInsets.left);
        
        // the farthest right the frame of an item in this section can go
        const CGFloat maxX = width - edgeInsets.right;
        ...
        for (NSInteger item = 0; item < itemCount; item++) {
            ...
            IGAssert(size.width <= paddedWidth, @"Width of item %zi in section %zi must be less than container %.0f accounting for section insets %@",
                     item, section, width, NSStringFromUIEdgeInsets(edgeInsets));
            ...
            const CGRect frame = IGListRectIntegralScaled(CGRectMake(itemX,
                                                                     itemY + (layingOutFirstLine || section == 0 ? edgeInsets.top : insets.top),
                                                                     itemWidth,
                                                                     size.height));
            ...
            nextRowY = MAX(CGRectGetMaxY(frame) - (layingOutFirstLine || section == 0 ? edgeInsets.top : insets.top), nextRowY);
        ...
        sectionData[section].edgeInsets = edgeInsets;
